...
| `KEELSON_OSDU_PMC_URL` | the PMC quality report's Pages site | Base URL for the Release Train grid's Dev Daily, QA Dev, Libraries, and Run History links. |
| `KEELSON_OSDU_ANALYTICS_URL` | the test-reliability analytics Pages site | Base URL for the grid's Test Reliability link (a separate Pages site). |
| `KEELSON_OSDU_BUNDLE_TTL_MS` | `600000` (10 min) | How long the shared activity fetch is cached before a re-fetch. |
| `KEELSON_OSDU_REPORT_TTL_MS` | `600000` (10 min) | How long the shared `osdu-quality release` report is cached before a re-fetch. `0` disables the cache. |
