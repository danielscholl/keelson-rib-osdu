...
(`M27 — Release 0.30 (Venus · Preview 2)`). Two columns:
...
A **PMC Report** strip trails the columns: one compact grid of labelled links to the
PMC dashboard's surfaces (`Status Summary`, `Analytics`, `Release Reports`, `Status
Reports`, `History`, `Smoke Tests`). It sits last because those links are constants —
the panel leads with what changes.
