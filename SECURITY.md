...
Thanks for taking the time to make the OSDU rib safer.

## Supported versions

The OSDU rib is pre-1.0 (`0.x`) software. Security fixes land on the
...
- Email: **degnome@gmail.com** with subject line `[osdu security]`
- GitHub private vulnerability report:
  <https://github.com/danielscholl/keelson-rib-osdu/security/advisories/new>

A useful report includes:

- A description of the issue and the impact you observed (or believe is
  possible)
- The OSDU rib version, your Keelson version (`keelson version --json`),
...
OSDU is a **Keelson rib** — a capability package installed into the
Keelson harness and discovered at boot. It reconciles OSDU platform
clusters, reads platform telemetry, and calls provider APIs. A rib runs
with the same privileges as the harness, so the threat model assumes:
...
- Hostile inputs may arrive over the network from provider responses,
  cluster/platform API responses, tool outputs, or fetched URLs.

### In scope

- Command injection or path traversal in any rib handler that splices
  untrusted data (provider output, cluster/platform API responses,
  upstream node output, workflow YAML fields) into a shell, child
  process, or filesystem path
- Credential or secret leakage: any path where the rib logs, snapshots,
  or transmits provider tokens, cluster credentials (kubeconfig, platform
  tokens), or other secrets it handles
